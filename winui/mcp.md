---
layout: post
title: Syncfusion WinUI MCP Server | Syncfusion
description: Learn how to configure and use Syncfusion WinUI MCP server for intelligent code generation, documentation, and troubleshooting in WinUI apps.
platform: winui
...
You can install the Syncfusion WinUI MCP server as a local tool without a global installation. For guidance on installing and managing local .NET tools, refer the [documentation](https://learn.microsoft.com/en-us/dotnet/core/tools/local-tools-how-to-use).

1. Install the Syncfusion WinUI MCP tool locally:
...
**Verifying Installation** Check your editor's MCP Server list for `sf-winui-mcp` with a **Connected** status to confirm a successful installation.

## Common use cases

The examples below showcase how the `search_docs` tool handles real-world WinUI development scenarios. The tool can be invoked directly, as shown in the examples below, for specific needs. Alternatively, an AI assistant can automatically invoke it based on the request.
...
#search_docs How do I set up the Syncfusion WinUI RichTextEditor with a custom toolbar and form integration?
{% endpromptcard %}
{% endpromptcards %}
...
#search_docs Why aren't my Scheduler appointments displaying or being saved correctly?
{% endpromptcard %}
{% endpromptcards %}

## Best Practices

To get the most out of the Syncfusion<sup style="font-size:70%">&reg;</sup> WinUI MCP Servers:

- **Be Specific** - Include the platform and component in your queries (for example, *"Create a Syncfusion WinUI Grid with sorting and filtering"*).
...
- **Stay Consistent** - Keep file organization, naming conventions, and coding standards consistent throughout your WinUI project.
...
- **For Troubleshooting** - Use AI suggestions for common issues; consult the [official documentation](https://help.syncfusion.com/winui/overview) or [support](https://support.syncfusion.com/support/tickets/create) for complex problems.
