...
Set the page's `DataContext` in code-behind and bind the `Conversations` property to display the archived conversations in the navigation view. Set `ShowNavigationView` to `True` because the navigation view is hidden by default.

{% tabs %}
{% highlight xaml tabtitle="MainPage.xaml" hl_lines="13 14 15 16" %}

<Page
    x:Class="AIAssistViewHistoryWinUI.MainPage"
...
![Conversation history in WinUI AI AssistView](aiassistview_images\winui_aiassistview_history.png)
