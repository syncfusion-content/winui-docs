...
You can show week number in `Calendar` control by setting the `ShowWeekNumbers` property value as **true**.
`Calendar` control also allows you to customize the template of the week numbers using `WeekNumberTemplate` property in `CalendarItemTemplateSelector` class. The default value of `ShowWeekNumbers` property is **true**.
...
![WinUI Calendar Themes](UI-Customization_images/winui-calendar-weeknumbertemplate.png)
...
![WinUI Calendar Themes](UI-Customization_images/winui-calendar-weeknametemplate.png)